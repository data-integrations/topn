...
public class TopN extends BatchAggregator<Boolean, StructuredRecord, StructuredRecord> {
  private static int MAX_TOP = 1024*1024;
  ...
  public void configurePipeline(PipelineConfigurer configurer) {
    ...
    if (field == null) {
      throw new IllegalArgumentException(
        String.format(
          "Field '%s' you have specified does not exist in the input.", topField
        )
      );
    } else {
      ...
      if (type != Schema.Type.INT && type != Schema.Type.LONG && type != Schema.Type.FLOAT &&
        ...
        throw new IllegalArgumentException(
          String.format(
            "Field '%s' is of type '%s', only types allowed are numeric types.", topField, type.toString()
          )
        );
      }
    }
    ...
    if (topSize > MAX_TOP) {
      throw new IllegalArgumentException(
        String.format(
          "Size specified '%d' is greater than max allowed '%d'", topSize, MAX_TOP
        )
      );
    }
