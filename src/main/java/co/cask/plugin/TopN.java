...
public class TopN extends BatchAggregator<Boolean, StructuredRecord, StructuredRecord> {
  ...
  public void configurePipeline(PipelineConfigurer configurer) {
    ...
    // First we check the top Field and validate it's type from the input schema.
    ...
    if (field == null) {
      throw new IllegalArgumentException(
        String.format(
          "Field '%s' you have specified does not exist in the input schema.", topField
        )
      );
    } else {
      ...
      if (type != Schema.Type.INT && type != Schema.Type.LONG && type != Schema.Type.FLOAT &&
        ...
        throw new IllegalArgumentException(
          String.format(
            "Field '%s' is of type '%s', only types allowed are number types.", topField, type.toString()
          )
        );
      }
    }
    ...
    if (topSize < 1) {
      throw new IllegalArgumentException(
        String.format(
          "Maximum number of top records to generated by sorting field '%s' is '%d', it should be greater than 1.",
          topField, topSize
        )
      );
    }

    // We check if boolean is set correctly or not by just invoking the method
    conf.getIgnoreNull();
